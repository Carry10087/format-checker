...
### 本轮专注任务（只检查这些，其他问题留给下一轮）

**必须逐项检查并修正：**

1. **首段格式（最重要）**：
   - **首段只能是一句话**：`主语 + is/are + ***定义*** + 句号`，结束
   - 第二句及之后的内容必须下沉到正文的四级标题下
   - 冠词(a/an/the)必须在 `***` 内部
   - 系动词(is/are/was)必须在 `***` 外部
   - 首段第一个单词不能加引号
   - ❌ 错误：`X is ***a brand***. It focuses on...`（两句话）
   - ✅ 正确：`X is ***a brand focused on Y***.`（一句话）

2. **引用位置**：
   - 所有 `[Note X](#)` 必须移到段落/列表项末尾
   - 严禁出现在句子中间
...
1. **粗体使用规范**：
   - 只有关键实体（人名、地名、作品名、型号）可以加粗
   - 禁止整句、形容词、动词加粗

2. **短信息合并**：
   - Social Media、基本参数等简短键值对合并为一行
   - 格式：`**INS**: @a, **Twitter**: @b`

3. **术语一致性**：
   - 全文专有名词写法必须统一（U.S. vs US 选一个）
   - 检查缩写是否前后一致
...
1. **只输出最终 Markdown 内容（V3终稿）**
2. **禁止任何解释、注释、说明**
3. **禁止用代码块包裹**
4. **直接输出纯净的 Markdown 正文**"""
]
