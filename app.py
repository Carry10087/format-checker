...
1. **首段格式（最重要）**：
   - 首段只能是一句话：`主语 + is/are + ***定义*** + 句号`，结束
   - 第二句及之后的内容必须下沉到正文
   - 冠词(a/an/the)必须在 `***` 内部，系动词在外部
   - 首段第一个单词不能加引号

2. **引用位置**：所有 `[Note X](#)` 移到段落/列表项末尾

3. **四级标题结构**：`#### Title` 下方必须直接跟列表，禁止段落
...
1. **粗体规范**：正文内容一律禁止加粗，只有列表小标题可以用 `**Key**:`

2. **短信息合并**：Social Media 等简短键值对合并为一行

3. **术语一致性**：全文专有名词写法统一（U.S./US 选一个）

4. **列表层级**：二级列表缩进4空格，分点不超过6个

5. **引用完整性**：四级标题下每个列表项必须有 `[Note X](#)`

6. **内容质量**：
   - 禁止过度关联（不能将概念与非固有的象征意义绑定）
   - 网友创作、非正式出版物不作为主要内容
...
|--------|------|
| 语言 | 全英文，无中文夹杂 |
| 政治正确 | Taiwan 后必须加 China |
| 首段 | 只能一句话，`***` 包裹正确，首词无引号 |
| 引用 | 全部在段末/列表项末，格式 `[Note X](#)` |
| 列表 | 有小标题、有句号、正文无加粗 |
| 场景规则 | YMYL有免责、玄学有娱乐提示 |
| 内容筛选 | 精确匹配优先、无过度关联、无低质来源 |
