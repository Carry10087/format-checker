...
### 2.5 标点符号规范
- **引号内标点**：逗号和句号必须包含在引号内，不能放在引号外。
  - ❌ 错误：`"Heart Signal 8", the show` / `"example".`
  - ✅ 正确：`"Heart Signal 8," the show` / `"example."`
...
- **搜索词匹配优先级**：当参考笔记中存在多个候选实体时，必须优先选择与搜索词拼写完全一致的实体。仅部分单词相同但整体名称不同的实体视为无关内容，必须排除（如搜索 "Super Idol" 时，"Super Junior" 的关联不是很大）。拼写不完全一致的实体（如搜索词为"Desire 4"但实体名为"Dzire"）应视为弱匹配，若存在精确匹配的实体则排除弱匹配内容。
