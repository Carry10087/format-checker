...
- **正文适度高亮（关键实体）**：
  - **允许**：为了方便快速扫描，**允许**在正文/列表中对**关键实体**（如人名、地名、作品名、特定型号、核心术语）进行加粗。
  - **禁止**：**严禁**对整句话、长短语、形容词或普通动词进行加粗。
  - 示例：
    - ❌ 滥用：`- **Artist**: **He was a great painter who lived in Paris.**` (整句加粗)
    - ❌ 滥用：`- **Artist**: He was a **very famous and talented** painter.` (形容词加粗)
    - ✅ 正确：`- **Artist**: **Jean Dunand** was a master of **lacquerware**, working in **Paris**.` (关键实体加粗)
